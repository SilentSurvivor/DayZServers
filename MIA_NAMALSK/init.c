...
class CustomMission: MissionServer
{
	override void OnInit()
	{
		...
		if ( m_EventManagerServer )
		{
			...
			m_EventManagerServer.RegisterEvent( Aurora, 0.88 );
			m_EventManagerServer.RegisterEvent( Blizzard, 0.2 );
	...
	override void UpdatePlayersStats()
	{
		...
		for (int i = 0; i < players.Count(); i++)
		{
			...
			if (!PlayerBase.CastTo(player, players.Get(i)))
				continue;
			// update player's dogtag based on time
	...
	override Dogtag_Base EquipDogtag(PlayerBase player)
	{
		...
		{
			// create a new tag if the player doesn't have one
